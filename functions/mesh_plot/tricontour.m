function h=tricontour(tri,nodes,z,nv,height,linewidth)
%TRICONTOUR Triangular Contour Plot.
% TRICONTOUR(TRI,[X,Y],Z,N,height,linewidth) draws scalar N contour lines treating the values
...
zs=z(tri);
zmax=max(zs(:));              % find max and min in z data that is in tri
zmin=min(zs(:));
...
for v=1:nlev                  % one contour level at a time
   ...
   while any(flag)	% while there are still unused edges -----------------
      ...
      ytmp(k+1:end)=[];                % xtmp,ytmp contain subcontour
              ...
              % plot the subcontour
         h=[h;patch('XData',xtmp,'YData',ytmp,'ZData',height(ones(k,1)),'CData',zc(ones(k,1)),...
         'Parent',hax,'FaceAlpha',0,'EdgeColor','flat','EdgeAlpha',0.7,'LineWidth',linewidth)];
